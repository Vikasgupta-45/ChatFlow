...
const SideBar = ({ isMenuOpen, setIsMenuOpen }) => {
  ...
  const { chats, setSelectedChat, theme, setTheme, user, navigate, createNewChat, axios, setToken, fetchUserChats, selectedChat, setChats } = useAppContext();
  const [search, setSearch] = useState("");
  const [deleteModal, setDeleteModal] = useState({ isOpen: false, chatId: null, chatName: "" });
  ...
  const deleteChat = (e, chat) => {
    e.stopPropagation();
    const chatName = chat.messages.length > 0
  ...
  const confirmDelete = async () => {
    try {
      const { data } = await axios.delete(`/api/chats/delete/${deleteModal.chatId}`, {
        headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
      });
      if (data.success) {
        setChats(prevChats => prevChats.filter((chat) => chat._id !== deleteModal.chatId));
        await fetchUserChats();
        toast.success(data.message);
        setDeleteModal({ isOpen: false, chatId: null, chatName: "" });
        // If the deleted chat was selected, clear selection or navigate
        if (selectedChat?._id === deleteModal.chatId) {
          navigate("/");
  ...
  return (
    ...
    >
      ...
      {/* {New Chat Button}       */}
      <button onClick={() => createNewChat()} className='flex justify-center text-xl active:scale-97
      ...
      <div className='flex-1 overflow-y-scroll mt-3 text-sm space-y-3'>
        {
          chats.filter((chat) => chat.messages[0] ? chat.messages[0]?.content.toLowerCase().includes(search.toLowerCase()) : chat.name.toLowerCase().includes(search.toLowerCase())).map((chat) => (
            <div
              ...
              key={chat._id}
              className={`p-2 mt-2 px-4 rounded-md cursor-pointer active:scale-98 flex justify-between group
                ${selectedChat?._id === chat._id
                  ? 'bg-gray-200 dark:bg-gray-800 border border-purple-500'
            ...
            >
              ...
              <img onClick={(e) => deleteChat(e, chat)} src={assets.bin_icon} className='hidden group-hover:block w-4 cursor-pointer not-dark:invert' alt='' />
            </div>
          ))}
      </div>
      ...
      <div className='flex items-center gap-4 p-3 mt-3  border border-gray-300 dark:border-white/15 rounded-md cursor-pointer hover:scale-103 group transition-all'>
        ...
        <p className='flex-1 text-sm drak:text-primary truncate'>{user ? user.username : "Login your account"}</p>
